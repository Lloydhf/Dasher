...
def build(output=None, qa=False):
    ...
    camera_look_at(cam,(66,115,225),(0,73,153))
    ...
    prop(lighting,'float','ExposureCompensation',0.1)
    for key,value in [('Brightness',2.4),('ClockTime',14.2),('EnvironmentDiffuseScale',0.65),('EnvironmentSpecularScale',0.75),('ShadowSoftness',0.3)]:
        prop(lighting,'float',key,value)
    prop(lighting,'bool','GlobalShadows',True)
    prop(lighting,'Color3','Ambient',{'R':127/255,'G':139/255,'B':151/255})
    prop(lighting,'Color3','OutdoorAmbient',{'R':157/255,'G':173/255,'B':185/255})
    atmosphere=item('Atmosphere','Atmosphere')
    for key,val in [('Density',0.16),('Offset',0.15),('Haze',0.6),('Glare',0.15)]: prop(atmosphere,'float',key,val)
    ...
    prop(starter,'float','CameraMaxZoomDistance',34)
    prop(starter,'float','CameraMinZoomDistance',7)
    prop(starter,'bool','EnableMouseLockOption',True)
