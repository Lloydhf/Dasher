...
def build(output=None, qa=False):
    ...
    # Deliberately small worlds: avoid streaming replication edge cases during first release.
    prop(ws,'bool','StreamingEnabled',False)
    lobby,maps=build_world()
    ws.append(lobby)
    cam=item('Camera','Camera')
    cframe(cam,'CFrame',(46,49,130),0.57)
    ...
    for key,value in [('Brightness',2.4),('ClockTime',14.4),('EnvironmentDiffuseScale',0.65),('EnvironmentSpecularScale',0.75),('ShadowSoftness',0.3)]:
        prop(lighting,'float',key,value)
    prop(lighting,'bool','GlobalShadows',True)
    prop(lighting,'Color3','Ambient',{'R':0.48,'G':0.56,'B':0.64})
    prop(lighting,'Color3','OutdoorAmbient',{'R':0.61,'G':0.69,'B':0.77})
    ...
    scripts.append(script('ModuleScript','ProfileStore','src/server/ProfileStore.luau'))
    server=script('Script','DasherServer','src/server/DasherServer.server.luau')
    if qa:
        source=server.find('Properties/ProtectedString')
        source.text='workspace:SetAttribute("DasherTestMode", true)\n'+source.text
